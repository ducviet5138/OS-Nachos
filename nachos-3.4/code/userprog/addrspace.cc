...
AddrSpace::~AddrSpace()
{
   for (int i = 0; i < numPages; i++) Frame->Clear(pageTable[i].physicalPage);
   delete pageTable;
}
...
// first, set up the translation 
    ...
    if (numPages > Frame->NumClear()) return;

    pageTable = new TranslationEntry[numPages];
    for (i = 0; i < numPages; i++) {
	pageTable[i].virtualPage = i;	// for now, virtual page # = phys page #
	pageTable[i].physicalPage = Frame->Find();
	pageTable[i].valid = TRUE;
	pageTable[i].use = FALSE;
	pageTable[i].dirty = FALSE;
	pageTable[i].readOnly = FALSE;  // if the code segment was entirely on 
...
// zero out the entire address space, to zero the unitialized data segment 
// and the stack segment
    for (int i = 0; i < numPages; i++) bzero(&(machine->mainMemory[pageTable[i].physicalPage * PageSize]), PageSize);
